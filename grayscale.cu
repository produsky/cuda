...
__global__ void grayscale_pixels(
    ...
    if(x < cols && y < rows) {
        int red = rgb[3 * (y * cols + x)];
        int green = rgb[3 * (y * cols + x) + 1];
        int blue = rgb[3 * (y * cols + x) + 2];

        grayscale_pixels_directive[y * cols + x] = (307 * red + 604 * green + 113 * blue) / 1024;
    }
}
...
int main() {
    ...
    cudaMemcpy(gray_pixels.data(), gray_pixels_directive, rows * cols, cudaMemcpyDeviceToHost);
